...
Along the way, the workshop teaches five practical agent-engineering skills: **skills design**, **context engineering**, **subagent handoff patterns**, **model routing**, and **evaluation**. Every exercise is measured against a fixed baseline (19,800 tokens, 1 call, Tier 3) so improvement is concrete and comparable across teams, and a leaderboard at the end scores both correctness and cost efficiency — not cost alone — because a fast, cheap agent that gets warranty decisions wrong is not actually a win.

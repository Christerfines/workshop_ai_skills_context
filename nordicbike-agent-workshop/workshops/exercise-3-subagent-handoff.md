...
One more thing worth knowing before you start: `.github/prompts/` only ships a starter scaffold for the triage half of this split (`warranty-triage.prompt.md`), plus two supporting helpers you'll use along the way (`policy-lookup.prompt.md`, and `escalation-router.prompt.md` — the latter belongs to Exercise 4, not this one). There is deliberately no starter file for the resolver. Reasoning over the payload and the cited policy text to actually reach and write an eligibility decision is the real design work this exercise is teaching, not plumbing to fill into a template — write that prompt yourself, in whatever form your setup uses, rather than looking for a fourth scaffold that was never meant to exist.

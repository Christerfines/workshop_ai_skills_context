...
Where does this go? **The exercise files describe what the deliverable must contain, not where to save it or how it reaches the facilitator** — the participant docs don't pin this down explicitly anywhere. Falcon does the sensible thing (and, not coincidentally, the same thing the maintainer's own test harness assumes): they save it as `deliverables/exercise-1.md` in their repo and commit it, so the facilitator can just read their repo directly later. A different team might paste it into a shared doc instead — nothing in the docs would tell them they're doing it "wrong." (Worth knowing as a real, small gap — flagged, not fixed, in this document.)
...
By now Falcon has five files sitting in `deliverables/` in their repo, committed as they went — this is, in effect, their submission; nothing in the participant docs describes a separate "hand this in" step beyond the repo/branch access the facilitator already has (the roster template in `facilitator-guide.md` literally has a "Repo / Branch" column, filled in during Phase 1). A team on a shared clone just needs their branch pushed; a team on individual forks needs the facilitator added as a collaborator or a PR opened — again, not something the docs pin down, so whichever the facilitator announced during Phase 1 setup is what actually happens.
...
Narrating this end to end surfaced one thing worth flagging, distinct from the two smaller fixes already made this session: **the participant-facing docs specify what each exercise's deliverable must *contain*, in detail, but never say where to save it or how a team actually gets it to the facilitator.** The roster template implies "the facilitator reads your repo," which is a coherent, workable answer — it's just implicit, reconstructed from `facilitator-guide.md`'s roster and Leaderboard sections rather than stated anywhere a participant would actually read. Not fixing this now since it wasn't what was asked — flagging it the same way the earlier Exercise 3 scaffold gap was flagged, in case it's worth a one-line addition to `README.md`'s "How to Run Each Exercise" later.

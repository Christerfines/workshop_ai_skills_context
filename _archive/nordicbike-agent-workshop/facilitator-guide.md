...
|---|---|---|---|
| 0 | `00-llm-basics-optional.mp3` | Optional — only for registrants without hands-on LLM-API experience (see below) | ~6 min |
| 1 | `01-meet-nordicbike.mp3` | Required | ~8 min |
| 2 | `02-the-domain.mp3` | Required | ~5 min |
| 3 | `03-context-engineering.mp3` | Required | ~7 min |
| 4 | `04-subagent-handoff.mp3` | Required | ~6 min |
| 5 | `05-model-routing.mp3` | Required | ~6 min |
| 6 | `06-evaluation-quality-gates.mp3` | Required | ~6 min |

Episodes 1–6 run roughly **40 minutes total** — mention that figure in the email itself, since "listen to a podcast series" undersells how small an ask this actually is, and a participant who thinks it's a bigger commitment than it is may just skip it.

- **Send with the same pre-workshop email** described in Setup Instructions below (the one carrying the Prerequisites section and the repository link), not as a separate follow-up — one email people actually read beats two they might not.
- **Attach the files or link to `pre_read_audio/` directly** — whichever your mail setup handles better with ~35–45 MB of attachments; a shared-drive link works just as well as attachments if your invite tool struggles with that size.
- **Send Episode 0 only to registrants you know are less LLM-experienced**, not the whole list — see `preparation/README.md`'s "Who needs Episode 0" for why blanket-sending it reads as condescending to the engineers who don't need it.
- **Do not attach or link anything else from this repo's facilitator-only material** in that same email — the pre-read series is deliberately spoiler-free (see `preparation/README.md`'s "What these are (and aren't)"), but `evaluation/`'s answer-key files and this guide itself are not, and belong nowhere near a participant inbox.
- **These MP3s are generated by a separate local tool, not part of this repo** (`~/proj/workshop_pre_read_audio` — see `preparation/README.md`'s "Generating the audio"). If you edit any `preparation/*.md` manus, regenerate the matching episode there and commit the refreshed MP3 here before it goes out — the two can drift out of sync silently otherwise.
...
## Spot-Checking for Genuine Engagement

Be honest with yourself about what this section can and can't do: there is no reliable way to prove a team personally engaged with each exercise rather than asking their AI tool to produce a finished-looking deliverable in one shot. This workshop's entire premise is "use AI well to build an agent," so there's no clean line between a team legitimately using Copilot to build their triage/resolver prompts — required, expected, the whole point — and a team asking it to skip straight to a plausible-looking V4. Nothing on disk tells the two apart with certainty. What follows is friction and spot-checkability, not proof, and it's worth knowing that going in rather than treating any of this as a guarantee.

That said, three things already work in your favor without any extra effort, and a few cheap habits catch most of what's left:

- **The Deliverable fields are already doing real work.** Exercise 2 wants the sections you cut and why; Exercise 3 wants both calls' actual token counts plus the real payload JSON; Exercise 4 wants a full 10-case routing log; Exercise 5 wants a full 10-case quality-gate table. A team that skipped the process has to backfill all of this convincingly across four exercises — that's more work than doing it for real, not less. You're already asking for these per the Leaderboard Running Procedure above; this is just a reminder that asking for them is itself the main defense, not a formality.
- **Watch artifact texture, not just artifact presence.** A genuine Exercise 3 payload has some rough edges — a field that's slightly over-included, a token count that isn't suspiciously round. Ten cases' worth of payloads that are all uniform, all land on the token target within a few tokens of each other, or all carry identically-phrased "why we cut this" notes reads like one batch-generated answer, not ten separately-reasoned ones.
- **During a Phase 4 or 5 walk-around, ask a team to explain one specific choice live** — "why does your triage payload have `water_exposure_reported` but no `manufacturing_defect` field?" A team that did the work answers in a sentence; a team that didn't visibly doesn't know their own deliverable.
- **If a submission looks too clean to be plausible, ask to see the raw session for one exercise** — the actual prompts and transcript, not the summarized deliverable — and, if you want to go further, re-run it yourself through `tests/claude/run.sh` or `tests/copilot/run.sh` against the same case to see whether the reported numbers reproduce. That harness already exists for content QA; pointing it at one suspicious submission instead of a fresh run costs nothing extra to build.
- **The scoring formula is already structurally hostile to the naive shortcut.** Q is 70% of FinalScore and a single adversarial "must not" violation is a flat 10-point Penalty no cost-efficiency gain can offset. A team that skipped the reasoning process is disproportionately likely to fail exactly the archetype cases — 1/2's symptom-vs-cause trap, 5's grandfather clause, 6/9's escalation triggers — because getting those right without doing the work is genuinely hard even for a capable model. The fiction's own difficulty is doing anti-shortcut work whether or not you do anything else in this section.
